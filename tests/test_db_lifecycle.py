...
def _pipeline_stage_specs(tmp_path: Path) -> list[StageSpec]:
    ...
    return [
        StageSpec(
            stage_name="scout",
            run_status="scout_complete",
            artifact_type="scout_report",
            artifact_path=tmp_path / "scout.json",
            create_run=create,
            start=lambda run_id: start_scout_run(settings, run_id=run_id),
            complete=lambda run_id, stage_id: complete_scout_run(
                settings,
                run_id=run_id,
                stage_id=stage_id,
                artifact_path=tmp_path / "scout.json",
            ),
            fail=lambda run_id, stage_id: fail_scout_run(
                settings,
                run_id=run_id,
                stage_id=stage_id,
                error="scout failed",
        ...
        StageSpec(
            stage_name="plan",
            run_status="plan_complete",
            artifact_type="implementation_plan",
            artifact_path=tmp_path / "plan.json",
            create_run=create,
            start=lambda run_id: start_plan_run(settings, run_id=run_id),
            complete=lambda run_id, stage_id: complete_plan_run(
                settings,
                run_id=run_id,
                stage_id=stage_id,
                artifact_path=tmp_path / "plan.json",
            ),
            fail=lambda run_id, stage_id: fail_plan_run(
                settings,
                run_id=run_id,
                stage_id=stage_id,
                error="plan failed",
