...
**RepairVerification** — The Skill used when a repair attempt runs after a verification failure. Primary instruction: "verification failed — fix the failing tests/build." Issue body is present as labeled background, not the primary task. `WorkOnIssue` is not loaded for repair attempts. Used for both per-issue repair loops and epic-level repair. Falls back to a hardcoded default if no skill file is found. See ADR 0018.
